-- Librarian: update library-branches, also in Administator procedures at the end.

DELIMITER $$

drop procedure if exists update_branch$$

CREATE PROCEDURE update_branch(IN pbranchId INT, IN pbranchName VARCHAR(45), IN pbranchAddress VARCHAR(45))
BEGIN
IF pbranchAddress <> 'N\A' THEN
	UPDATE tbl_library_branch
	SET branchAddress = pbranchAddress
	WHERE branchId = pbranchId;
END IF;
IF pbranchName <> 'N\A' THEN
	UPDATE tbl_library_branch
	SET
...
BEGIN
	...
	CASE param_operation
		WHEN 'A' THEN
			INSERT INTO tbl_publisher (publisherId, publisherName, publisherAddress)
            VALUES (biggest_ID, param_NEWpublisherName, param_NEWpublisherAddress);
...
if bookEmpty then
	if authEmpty then
		insert into tbl_book values (1, bTitle);
        insert into tbl_author values (1, aName);
        insert into tbl_book_authors values (1, 1);
        leave proc_label;
    else
